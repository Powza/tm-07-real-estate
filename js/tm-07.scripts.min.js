...
{
    //...
    if( wScrollCurrent <= 0 ) // scrolled to the very top; $('.header__meta') sticks to the top
        $('.header__meta').style.top = '0px';
 
    else if( wScrollDiff > 0 ) // scrolled up; $('.header__meta') slides in
        $('.header__meta').style.top = ( elTop > 0 ? 0 : elTop ) + 'px';
 
    else if( wScrollDiff < 0 ) // scrolled down
    {
        if( wScrollCurrent + wHeight >= dHeight - elHeight )  // scrolled to the very bottom; $('.header__meta') slides in
            $('.header__meta').style.top = ( ( elTop = wScrollCurrent + wHeight - dHeight ) < 0 ? elTop : 0 ) + 'px';
 
        else // scrolled down; $('.header__meta') slides out
            $('.header__meta').style.top = ( Math.abs( elTop ) > elHeight ? -elHeight : elTop ) + 'px';
    }
    //...
});
  ...
  // SmartMenus
  $(".header #main_navigation, .mobile-menu #main_navigation").smartmenus();
  ...
  function headerCalculateHeroPadding() {
    ...
    var heroCallout = $(".hero__callout").outerHeight();
    var mlsSearch = $(".mls-search").outerHeight() + header;
    var total = header + heroCallout;

    if ($(window).width() < 992) {
      $("body").css("padding-top", mlsSearch + "px");
      $(".hero__content").css("min-height", "calc(100vh - " + mlsSearch + "px)");
    } else if ($(window).width() > 993) {
      $("body").css("padding-top", header + "px");
      $(".hero__content").css("min-height", "calc(100vh - " + total + "px)");
    }
  }
