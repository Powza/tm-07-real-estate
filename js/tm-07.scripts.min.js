$(function () {
    ...
    function quickSearchSuggest() {
        $('#qs_suggest').each(function (i) {
            var qs_suggestId = "qs_suggest_" + i;
            this.id = qs_suggestId;
            $('.mls-loading-results').hide();
            magic_suggest = $(this).magicSuggest({
                hideTrigger: true,
                minChars: 2,
                maxSuggestions: 100,
                ...
                displayField: 'name',
                renderer: function (a) { if (typeof (a.display) == 'undefined' || a.display == '') { return a.id; } else { return a.display; } },
                selectionRenderer: function (a) { if (typeof (a.display) == 'undefined' || a.display == '') { return a.id; } else { return a.display; } }
            });
            $(magic_suggest).on('beforeload', function () {
                $(".mls-loading-results").show();
            });

            $(magic_suggest).on('load', function () {
                $(".mls-loading-results").hide();
            });
        });
    }
    ...
    function sidebarScrollList() {
        ...
        $(".sidebar__list-scroll .list-slider ul:first").each(function (i, item) {
            var slidesShowScroll = $(this).parent().data('list-items');
    ...
    function multilevelList() {
        $('.sidebar__multilevel-list').each(function (i) {
            ...
            $('ul:first', this).smartmenus('destroy').removeAttr('id class style').addClass('sm sm-vertical-list').smartmenus();
        });
    }
    multilevelList();
    ...
    function multiLevelMenusList() {
        ...
        $('.sidebar__multilevelmenus-list .panel-title a').each(function (i) {
            $(this).attr('href', '#collapse_' + i).attr('aria-controls', 'collapse_' + i).attr('aria-expanded', 'false');
        });
        ...
        $('.sidebar__multilevelmenus-list .panel-collapse').each(function (i) {
            $(this).attr('id', 'collapse_' + i).attr('aria-labelledby', 'heading_' + i);
            $('ul:first', this).smartmenus('destroy').removeAttr('id class style').addClass('sm sm-vertical-list').smartmenus();
        });
    }
    multiLevelMenusList();
    ...
    function propertySlider() {
        ...
        $('.property-slider:not(.sidebar .property-slider) .slider-wrap').each(function (i, item) {
            ...
            $(this).slick({
                ...
                appendArrows: $('<div></div>').addClass('section__slider-arrows').appendTo($(this).parent()),
                responsive: [
                    {
                        breakpoint: 1680,
                        settings: {
    ...
    function propertySliderSidebar() {
        $('.sidebar .property-slider').each(function (i) {
            ...
            $('.sidebar .property-slider#sidebar_slide_' + i + ' .slider-wrap').html($('.sidebar .property-slider#sidebar_slide_' + i + ' .item').sort(function () {
    ...
    function agentSlider() {
        $('.agent-slider:not(.sidebar .agent-slider)').each(function (i) {
            ...
            if (agentSort.toLowerCase() == 'random') {
                $('.agent-slider:not(.sidebar .agent-slider)#slide_' + i + ' .slider-wrap').html($('.agent-slider:not(.sidebar .agent-slider)#slide_' + i + ' .slider-wrap .agent').sort(function () {
                    return Math.random() - 0.5;
                }));
            }
    ...
    function agentSliderSidebar() {
        $('.sidebar .agent-slider').each(function (i) {
            ...
            let agentSort = $.trim($(this).data('agent-sort'));
            if (agentSort.toLowerCase() == 'random') {
                $('.agent-slider#sidebar_slide_' + i + ' .slider-wrap').html($('.agent-slider#sidebar_slide_' + i + ' .slider-wrap .agent').sort(function () {
                    return Math.random() - 0.5;
                }));
            }
            let agentPrimary = $.trim($(this).data('agent-primary'));
    ...
    function agentTestimonials() {
        ...
        if ($('.testimonials-slider .slider-wrap iframe').length > 0) {
            function delayLoad() {
                $('.testimonials-slider .slider-wrap .slick-list').css('height', 'auto');
            };
    ...
    function heroImage() {
        $('.hero__media__slider .slider').slick({
            ...
            appendArrows: $('<div></div>').addClass('section__slider-arrows hidden-xs').appendTo('.hero__content__wrap'),
        });
    }
