...
function areaPolygonMap() {
  var mapOptions = {
    zoom: 10,
    gestureHandling: "greedy",
    center: new google.maps.LatLng(33.680346, -78.951573),
    fullscreenControl: true,
    fullscreenControlOptions: {
      position: google.maps.ControlPosition.LEFT_TOP
    }
  };
  map = new google.maps.Map(document.getElementById("areaPolygonMap"), mapOptions);
}

/* Get polygon data */
$.ajax({
  url: "js/gene_carter_custom/area-polygon-map-coords.js",
  type: "GET",
  dataType: "json",
  success: function(data) {
    $.each(data, function(i) {
      polygonInitialize(data[i]);
    });
  },
  error: function(e) {
    console.log(e);
...
/* Confiure polygon data */
function polygonInitialize(data) {
  ...
  for (var i = 0; i < polygon.coordinates.length; i++) {
    polyCoordinates.push(
      new google.maps.LatLng(parseFloat(polygon.coordinates[i].lat), parseFloat(polygon.coordinates[i].lng))
    );
  ...
  var polygonSet = new google.maps.Polygon({
    ...
    strokeOpacity: polygon.strokeOpacity ? polygon.strokeOpacity : 0.8,
    strokeWeight: polygon.strokeWeight ? polygon.strokeWeight : 1,
    fillColor: polygon.fillColor ? polygon.fillColor : "#FF0000",
    fillOpacity: polygon.fillOpacity ? polygon.fillOpacity : 0.35,
    zIndex: 1,
    clickable: false,
    map: map
  });
  ...
  var marker = new MarkerWithLabel({
    ...
    map: map,
    icon: " ",
    labelContent: label.name ? label.name : "",
    labelAnchor: label.anchor
      ? new google.maps.Point(label.anchor.xAxis, label.anchor.yAxis)
      : new google.maps.Point(40, 20),
    labelClass: "area-polygon-map-marker",
    labelInBackground: true
  });
  /* Hover label */
  google.maps.event.addListener(marker, "mouseover", function() {
    polygonSet.setOptions({
      fillColor: "#999",
      zIndex: 2
    });
  });
