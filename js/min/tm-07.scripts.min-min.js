...
// Navbar Scroll
function l(){var e=document.querySelector(".header .header__meta"),t;window.location.hash&&e.classList.add("headroom--unpinned"),new Headroom(e,{tolerance:{down:10,up:20},offset:$(".admin-navbar").outerHeight()+$(".header").outerHeight()}).init()}
// Header padding
...
// Hero Video
function b(){var e=".hero__video";$(e).YTPlayer({videoURL:$(e).data("video-url"),containment:e,loop:!0,autoPlay:!0,showControls:!1,abundance:0,stopMovieOnBlur:!1,playOnlyIfVisible:!0,onScreenPercentage:100})}
// Check if media type is defind and default if empty
function m(){var e=$.trim($(".hero__media").data("media-type"));"video"==e?($(".hero__media__slider").remove(),$(".hero__media__video").show(),b()):($(".hero__media__video").remove(),$(".hero__media__slider").show(),p())}function h(){
//set the index counter
var i=0;
//convert all .nav-tabs, except those with the class .keep-tabs
$("#n .nav-tabs").not(".keep-tabs").each(function(){
//init variables
var e,t=0,e=$('<select class="mobile-nav-tabs-'+i+' mobile-tab-headings "></select>');
...
$(this).children("li").each(function(){e.append('<option value="'+t+'">'+$(this).text()+"</option>"),t++}),
...
classArray=$(this).attr("class").split(" "),tabIndexArray=classArray[0].split("mobile-nav-tabs-"),tabIndex=tabIndexArray[1],
//using boostrap tabs, show the selected option tab
$(".nav-tabs-index-"+tabIndex+" li a").eq($(this).val()).tab("show")})}e(),
// Lazyload
$(".lazy").Lazy({scrollDirection:"vertical",threshold:0}),t(),i(),s(),r(),o(),a(),n(),
// svg4everybody
svg4everybody(),
// SmartMenus
...
$("#qs_form .multi-select.placeholder").remove(),$("#qs_form .multi-select").show(),$("#qs_form .multi-select").multipleSelect({minimumCountSelected:2,placeholder:$(this).data("placeholder")}),l(),$(window).resize(function(){d()}),d(),c(),
// Enable tooltips
$('[data-toggle="tooltip"]').tooltip({delay:{show:500,hide:500}}),u(),m(),h()});